...
export async function destroy(id){  
    ...
    if (resp.status !== 204) return {error: "Erro ao apagar produto. " + resp.status}
...
export async function update(produto){
    ...
    if (resp.status !== 200) return {error: "Erro ao atualizar produto. " + resp.status}
...
export async function getProduto(id){
    ...
    if (resp.status !== 200) return {error: "Erro ao buscar dados do produto. " + resp.status}
    const json = await resp.json()
    return json
}